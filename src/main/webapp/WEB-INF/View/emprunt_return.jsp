                ...
                <div class="row">
                  <div class="input-field col s12">
                    <select id="id" name="id" class="browser-default">
                      ...
                      <c:if test="${!listCurrent.isEmpty()}">
                        <c:forEach items="${listCurrent}" var="emprunt">
                          <c:if test="${empruntById.id==emprunt.id}">
                            <option value="${emprunt.id}" selected>"${emprunt.livre.titre}", ${emprunt.membre.prenom}
                              ${emprunt.membre.nom}
                            </option>
                          </c:if>
                          <c:if test="${empruntById.id!=emprunt.id}">
                            <option value="${emprunt.id}">"${emprunt.livre.titre}", ${emprunt.membre.prenom}
                              ${emprunt.membre.nom}</option>
                          </c:if>
